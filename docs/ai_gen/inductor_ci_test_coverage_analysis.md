...
**口径警示 —— 该比值不是用例适配覆盖率**。分子(NPU 自建)与分母(社区用例)不是
同一批用例,二者"1 个用例"不可交换: 验证目标(上游 opinfo 是编译后 vs eager 全
dtype×多组 sample 对拍 + 反向 + 无-fallback 检查,NPU 自建多为单算子 smoke)、断言
强度、输入空间、反向覆盖均不同。且 NPU 自建用例几乎无参数化(静态≈执行),而社区
静态数被参数化严重稀释,静态比会**系统性高估**: P0 按静态 5.3%,按上游实跑分母
67/6,427 ≈ **1.0%**;域10 按静态 69.9%,按实跑 86/5,776 ≈ **1.5%**。本表只回答两件事:

1. **域覆盖广度**(定性): 13 域中 12 域已有自建看护(仅分布式为 0);
2. **域投入配比**(相对): 各域自建规模的相对大小 —— NPU 投入集中在编译流程可用性
   (动态形状/代码生成/融合 pass),P0 正确性矩阵投入最薄。

**真正的"NPU 适配社区用例覆盖率"当前不可计算**: 它 = 按 §6.1 device 注入跑社区
用例集后的通过/跳过/失败统计,而 torch_npu 对上游 inductor 用例 fork 为 0、尚未
做注入统计,该值是"未测"而非 8.9%。§6 落地机制的第一批产出就应是这个数。
