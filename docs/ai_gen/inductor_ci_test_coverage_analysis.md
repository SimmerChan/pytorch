...
3. 合理需求口径是**"设备无关用例集通过率"**;建议按 P0~P3 分层接入,P0 = 算子级正确性基线
   ...
   "广度+正确性"双看护。预估 P0~P2 覆盖 CUDA 用例主体(约 75%),P3 与"不做集"明确排除。
...
预估(静态方法口径):P0+P1 约 1940 个(30%,且 opinfo 运行时展开后实际占比更高),叠加
P2 后约 3010(47%),再加 P3 约 5460(86%);剩余为"不做集"(601)与逐例判定的硬件敏感
用例、分布式(50)等专项。
...
- 建议按 P0(算子正确性+fallback 收敛)→ P1(动态形状+融合调度)→ P2(autotune/graph/训练)
  → P3(flex/AOTI/工具链)分层接入,厂商专属改为昇腾等价自建用例;
